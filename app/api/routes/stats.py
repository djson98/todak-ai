...
def get_stats(user_id: str, period: Literal["week", "month"] = "week"):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"통계 조회 실패: {error_msg}")
...
def get_report(user_id: str, period: Literal["week", "month"] = "week"):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"레포트 생성 실패: {error_msg}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"레포트 생성 실패: {str(e)}")
